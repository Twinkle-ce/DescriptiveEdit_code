...
from attention_bridge.utils import isinstance_str, init_generator
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    x = dropout(x)
                    result_lora = result + self._apply_dora(
                        x, lora_A, lora_B, scaling, active_adapter
                    )
                ### add mask for adapter
                lora_mask = self.lora_mask[active_adapter]
                lora_mask = lora_mask.repeat_interleave(
                    x.shape[0] // len(lora_mask), dim=0
                )
    ...
    class ToMeBlock(block_class):
                    ...
                    for layer_name in module.adapter_layer_names:
                        ...
                        for key, layer in module_dict.items():
                            if key == joint_adapter_name:
                                attn1_state_dict = attn1_module_dict[
                                    adapter_name
                                ].state_dict()
        ...
        def forward(
            ...
            if self.enable_joint_attention:
                ...
                lora_mask = lora_mask.repeat_interleave(
                    batch_size // len(lora_mask), dim=0
                )

                joint_encoder_hidden_states = torch.zeros_like(norm_hidden_states)
                joint_encoder_hidden_states[~lora_mask] = norm_hidden_states[lora_mask]
                joint_encoder_hidden_states[lora_mask] = norm_hidden_states[lora_mask]

                attn_output1n_ = self.attn1n(
                    norm_hidden_states,
                    encoder_hidden_states=joint_encoder_hidden_states,
                    attention_mask=attention_mask,
                    **cross_attention_kwargs,
                )
                attn_output = attn_output + self.conv1n(attn_output1n_)
        ...
        for name, module in diffusion_model.named_modules():
            if mode == "downmid":
                if name.startswith("up_blocks"):
                    logger.info(f"Pass {name} in the mode {mode}")
                    continue
            elif mode == "midup":
            ...
            if with_temporal_block and isinstance_str(
