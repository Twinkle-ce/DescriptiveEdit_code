...
class StableDiffusionPipelineMultiModel(
    DiffusionPipeline,
    ...
    def __call__(
        ...
        **kwargs,
        ...
        ref_image_latents = self.vae.encode(
            image.to(dtype=self.vae.dtype, device=self.vae.device)
        ).latent_dist.mean  # (bs, c, height, width)
        ...
        latents[batch_size // 2 :] = ref_image_latents
        # 7. Denoising loop
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                patch_joint_attention.set_joint_attention(self.unet, enable=True)
                noise_pred_joint = self.unet(
                    latent_model_input,
                    torch.tensor([t, torch.zeros_like(t), t, torch.zeros_like(t)])
                    .repeat_interleave(len(latents) // 2)
                    .to(latents.device),
                ...
                patch_joint_attention.set_joint_attention(self.unet, enable=False)
                noise_pred_separate = self.unet(
                    latent_model_input,
                    torch.tensor([t, torch.zeros_like(t), t, torch.zeros_like(t)])
                    .repeat_interleave(len(latents) // 2)
                    .to(latents.device),
                ...
                if self.do_classifier_free_guidance:
                    if t > 0:
                        noise_pred_uncond_joint, noise_pred_text_joint = (
                            noise_pred_joint.chunk(2)
                        )
                        noise_pred_uncond_separate, noise_pred_text_separate = (
                            noise_pred_separate.chunk(2)
                        )
                        noise_pred = (
                            noise_pred_uncond_joint
                            + guidance_scale
                            * (noise_pred_text_joint - noise_pred_uncond_joint)
                            + joint_guidance_scale
                            * (noise_pred_text_joint - noise_pred_text_separate)
                        )
                    else:
                        noise_pred_uncond_separate, noise_pred_text_separate = (
                            noise_pred_separate.chunk(2)
                        )
                        noise_pred = noise_pred_uncond_separate + guidance_scale * (
                            noise_pred_text_separate - noise_pred_uncond_separate
                        )
                ...
                latents[: batch_size // 2] = self.scheduler.step(
        ...
        latents = latents[: latents.shape[0]]
