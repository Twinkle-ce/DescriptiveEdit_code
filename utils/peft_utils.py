...
def get_peft_model_state_dict(
    ...
    if config.peft_type in (PeftType.LORA, PeftType.ADALORA):
        ...
        to_return = {k: v for k, v in to_return.items() if (("lora_" in k and f".{adapter_name}." in k) or ("bias" in k))}
...
def set_adapters_requires_grad(model, requires_grad = True, adapter_names = None):
    for _, module in model.named_modules():
        if isinstance(module, BaseTunerLayer):
            if hasattr(module, "enable_adapters"):
                enable_adapters(module, enabled=requires_grad, adapter_names=adapter_names)
            else:
                # support for older PEFT versions
                module.disable_adapters = False

def enable_adapters(module, enabled: bool, adapter_names = None) -> None:
